...
pygame.display.set_icon(pygame.image.load('images/aiu_logo.jpg'))
...
bg = pygame.image.load('images/bg_aiu.jpg')
...
while run:
    ...
    if len(pipe_group) > 0:
        if bird_group.sprites(
                ...
                and bird_group.sprites(
                    )[0].rect.right < pipe_group.sprites(
                    )[0].rect.right \
                and pass_pipe is False:
            pass_pipe = True
        if pass_pipe is True:
            if bird_group.\
                    sprites()[0].rect.left > pipe_group.\
                    sprites()[0].rect.right:
                score += 1
                pass_pipe = False
    draw_text(str(score), font, white, int(screen_width / 2), 20)

    if pygame.sprite.\
            groupcollide(bird_group, pipe_group, False, False)\
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.\
                MOUSEBUTTONDOWN and flying is False and game_over is False:
            flying = True
